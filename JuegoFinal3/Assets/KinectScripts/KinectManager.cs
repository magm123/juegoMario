...
//	public bool NearMode = false;
	...
	public Quaternion GetUserOrientation(uint UserId, bool flip)
	{
		if(UserId == Player1ID && player1JointsTracked[(int)KinectWrapper.NuiSkeletonPositionIndex.HipCenter])
			return ConvertMatrixToQuat(player1Ori, (int)KinectWrapper.NuiSkeletonPositionIndex.HipCenter, true);
	...
	public void DetectGesture(uint UserId, KinectGestures.Gestures gesture)
	{
		...
		switch(gesture)
		{
            ...
            //case KinectGestures.Gestures.RightHandCursor:
              //  gestureData.checkForGestures.Add(KinectGestures.Gestures.RightHandCursor);
              //  break;
...
//				break;
	...
	void Awake()
	{
		...
		if(CalibrationText != null)
		{
			CalibrationText.GetComponent<GUIText>().text = "ESPERANDO A LOS USUARIOS";
		}
		
		Debug.Log("ESPERANDO A LOS USUARIOS");
				...
				foreach(KinectGestures.GestureData gestureData in player1Gestures)
				{
					if(gestureData.complete)
					{
						if(gestureData.gesture == KinectGestures.Gestures.Click)
						{
                            ...
                            {
                                GameObject boton = GameObject.Find("NuevoUsuario");
                                //Debug.Log("Posicion del objeto:" + boton.transform.position);
                                //Debug.Log("Posicion del click:" + gestureData.screenPos);
                                if ((gestureData.screenPos.x >= 0.41 && gestureData.screenPos.x <= 0.63))
                                {
