...
//	}
	...
	public static void CheckForGesture(uint userId, ref GestureData gestureData, float timestamp, ref Vector3[] jointsPos, ref bool[] jointsTracked)
	{
		...
		{
            case Gestures.Cuadrado:
                switch (gestureData.state)
                {
                    case 0:
                        if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] &&
                            (Mathf.Abs(jointsPos[rightShoulderIndex].y - jointsPos[rightElbowIndex].y) < 0.1f) &&
                            (Mathf.Abs(jointsPos[rightHandIndex].y - jointsPos[rightElbowIndex].y) < 0.1f) &&
                            (jointsPos[rightElbowIndex].x != jointsPos[rightShoulderIndex].x) && jointsTracked[leftHandIndex]
                            && jointsTracked[leftElbowIndex] && jointsTracked[leftShoulderIndex] &&
                            (Mathf.Abs(jointsPos[leftShoulderIndex].y - jointsPos[leftElbowIndex].y) < 0.1f) &&
                            (Mathf.Abs(jointsPos[rightHandIndex].y - jointsPos[rightElbowIndex].y) < 0.1f))
                        {
            ...
            case Gestures.LineaRecta:
                switch (gestureData.state)
                {
                    case 0:
                        if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] &&
                            (jointsPos[rightShoulderIndex].y > jointsPos[rightElbowIndex].y) &&
                            (jointsPos[rightHandIndex].y < jointsPos[rightElbowIndex].y) &&
                            (Mathf.Abs(jointsPos[rightElbowIndex].x - jointsPos[rightShoulderIndex].x) > 0.1f) 
                            && jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[leftShoulderIndex] &&
                            (jointsPos[leftShoulderIndex].y > jointsPos[leftElbowIndex].y) &&
                            (jointsPos[leftHandIndex].y < jointsPos[leftElbowIndex].y) &&
                            (Mathf.Abs(jointsPos[leftElbowIndex].x - jointsPos[leftShoulderIndex].x) > 0.1f))
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                        }
                        break;
                ...
                break;
            case Gestures.Prueba1:
                switch (gestureData.state)
                {
                    case 0:
                        if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] &&
                            (Mathf.Abs(jointsPos[rightShoulderIndex].y - jointsPos[rightElbowIndex].y)<0.2f) &&
                            (jointsPos[rightHandIndex].y > jointsPos[rightElbowIndex].y) &&
            ...
            case Gestures.Rectangulo:
                switch(gestureData.state)
                {
                    case 0:
                        if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex] &&
                            ...
                            (jointsPos[rightElbowIndex].x != jointsPos[rightShoulderIndex].x) && jointsTracked[leftHandIndex] 
                            && jointsTracked[leftElbowIndex] && jointsTracked[leftShoulderIndex] &&
                            (Mathf.Abs(jointsPos[leftShoulderIndex].y - jointsPos[leftElbowIndex].y) < 0.2f) &&
                            (jointsPos[leftHandIndex].y > jointsPos[leftElbowIndex].y) &&
                            (jointsPos[leftElbowIndex].x != jointsPos[leftShoulderIndex].x))
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightHandIndex+leftHandIndex, jointsPos[rightHandIndex+leftHandIndex]);
                        }
                        break;
                ...
                break;
            case Gestures.FormaL:
                switch(gestureData.state)
                {
                    case 0:
                        if (jointsTracked[rightHandIndex] && jointsTracked[rightElbowIndex] && jointsTracked[rightShoulderIndex]
                            && (Mathf.Abs(jointsPos[rightHandIndex].x - jointsPos[rightElbowIndex].x)<0.3f) && 
                ...
                break;
            case Gestures.FormaLInv:
                switch (gestureData.state)
                {
                    case 0:
                        if (jointsTracked[leftHandIndex] && jointsTracked[leftElbowIndex] && jointsTracked[leftShoulderIndex]
                            && (Mathf.Abs(jointsPos[leftHandIndex].x - jointsPos[leftElbowIndex].x) < 0.3f) &&
                            ...
                            && (Mathf.Abs(jointsPos[rightHandIndex].y - jointsPos[rightShoulderIndex].y) < 0.3f))
                        {
                            SetGestureJoint(ref gestureData, timestamp, rightHandIndex + leftHandIndex, jointsPos[rightHandIndex + leftHandIndex]);
                        }
                        break;
